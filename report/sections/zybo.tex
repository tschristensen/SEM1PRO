...
For instance, the PWM switching frequency should be 20\si{\kilo\hertz} as described in section ????.
...
When the software is finished tests should be performed to verify real time performance.
\todo[inline]{Thomas: I would assume that the software is finished when i read a report on the software?}
...
\todo[inline]{Thomas: This is the first time AXI is mentioned - reader is unaware of what that means.}
...
\todo[inline]{Thomas: Switching limit being the value of the counter where the switch happens?}
...
This is important as otherwise the dutycycle may be corrupted.
\todo[inline]{Thomas: How and why?}
...
<<<<<<< HEAD
=======
\todo[inline]{Thomas: The first two blocks in \ref{fig:pwm} seem to be broken?}
\todo[inline]{Thomas: I would like it if the signals in the diagrams were given space-less names: counter\_out}
...
\begin{figure}[!h]
	...
	\label{fig:interface}
\end{figure}
\todo[inline]{Is the inrush relay explained somewhere?? -MIkkel, \ref{sec:irrelay}}
...
Then the task will read all signals from the digital board and, if they are all good, the global enable variable will be set high.
If one or more of the drive signals indicate a fault, the task will change the enable variable to low.
\todo[inline]{Thomas: What are these drive signals?}
...
\todo[inline]{TThomas: Is the dq transform presented anywhere?}
$I_d$ and $I_q$ should be controlled towards a set point by two separate controllers.
The point for $I_d$ is 0 as will be explained later?? .
\todo[inline]{Thomas: I can't wait to see this explanation! Where is it!?} 
...
\todo[inline]{Thomas: Why is that preferable?}
...
\todo[inline]{Thomas: By definition, it is saturated.. Do you mean that they are limited?}
...
\todo[inline]{Thomas: I am not sure what to do about this section but it doesn't convey it's purpose very well.. I have no idea what the task does, other than it is related to PWM...}
...
\todo[inline]{Thomas: Are they in series?}
