...
An interface to a PC is established using UART.
This interface enables the setting of values within the system as well as potentially reading the current status of the system.
...
\begin{lstlisting}[captionpos=b,style=customCpp, caption={Something clever!.}, label=lis:cutcommand, escapeinside={(*@}{@*)}]
if(rx_flag)
{
...
Lines 4-8 performs anti windup and lines 9-14 uses the output if enable is high and performs integrator reset if low.

\begin{lstlisting}[captionpos=b,style=customCpp, caption={[C code constituting a PI controller.]C code constituting a PI controller with trapezoidal integration, anti windup and integration reset.}, label=lis:controller_code]
...
Afterwards third harmonic injection is performed and if the values are higher or lower than the bounds they are saturated.
The global enable variable is checked and if high the physical enable pin is set high.
