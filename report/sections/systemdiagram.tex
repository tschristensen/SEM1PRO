...
\todo[inline]{Mikkel: ADC is not +- 0.5 volt anymore}
\todo[inline]{Mikkel: We use some other ports on the encodder now.}
...
\todo[inline]{Mikkel: To the figure: The Zybo does not handle the driver.}
...
In order to fulfill the task there needs to be developed some electric circuits to perform certain tasks. 
...
There needs to be an embedded system in the project in order to perform certain tasks.
The embedded system was found to be responsible for the following functionalities:
...
The subject of power electronics needs to be utialized in order to build a three phase inverter. 
The functionality of the inverter is to convert the DC voltage of the batteries into a three phase sinusoidal singal needed to run the motor.
...
Taking all aspects, such as switching losses and heat dissipation, into account and still producing a capable drive signal, is the main focus of section \ref{sub:inverter}.
The development of the power electronics is discussed in detail in section \ref{sec:power}.

\subsection{PMAC Motor}
The provided PMAC motor needs to be analysed in order to drive it optimally and simulate the performance of it. 
Parameterisation of the motor needs to done as a part of the analysis. 
