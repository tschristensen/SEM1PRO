...
\todo[inline]{Mikkel: This needs to be explained a bit better. May I give it a try? }
...
\todo[inline]{Mikkel: Maybe we should just talk about what problems that has to be handled and not how we did it yet.}
These three boards must implement the functions listed here:
\todo[inline]{Mikkel: Battery undervoltage protection is done by the supervisors?.}
\todo[inline]{Mikkel: Currente transducer = LEM right? Maybe it is better to write this}
\begin{itemize}
...
\todo[inline]{Mikkel: I don't think it should be specific yet. Only a listing of task to do..}
\begin{itemize}
...
\item Controller. The digital controller used to control the entire system will be implemented using the processer in the system
\item Interface for peripherals. A number of switches, LED's, enable and fault signals are used in the control of the motor. The Zybo must implement the necessary interfaces in order to properly handle these signals.
...
\todo[inline]{Mikkel: Propel? And I think this section should be more like the other}
...
\todo[inline]{Add something about simulation and control- Mikkel}