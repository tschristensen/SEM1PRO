...
Since all of the control is done digitally, obviously, there has to be a coupling between GNDD and GNDA at some point.
In order to minimize noise propagation there will be only one such point
...
The resistance of PEDAL varies from $0$ to $7.5 k\Omega$, as the pedal is pressed down. 
...
\todo{Thomas: Naming is not a mistake, this is the circuit that compares the temperature, current and zybo enable signals}
