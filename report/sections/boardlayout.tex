...
An extended discussion of the grounding planes as well as the actual layout can be found in appendix \ref{app:boardlayout}.
...
This sets $R_6 = 22.5k$\si{\kilo\ohm}.
...
As described in section \todo[inline]{Thomas: section on the zybo adc}, the ADC on the Zynq chip can work on signals in the ranges 0\si{\volt}-1\si{\volt} or $\pm0.5$\si{\volt}.
...
Since $R_{ds(on)}$ is known from the MOSFETs the current $I_{ds}$ can be determined:
\begin{equation}
	I_{ds} = \frac{V_{ds}}{R_{ds(on)}}
\end{equation}
...
If the received packet is of type WRITE, the data in the response will contain the contents of status register 1
...
\begin{figure}
	...
	\begin{subfigure}[b]{\linewidth}
		...
		\label{fig:statreg2}
	\end{subfigure}
	\caption{These registers contain the error bits of the system. The status of FAULT is available on the corresponding pin. If either of GVDD\_UV, GVDD\_OV or PVDD\_UV are in error, the PWRGD pin is pulled low. Finally, the OCTW pin can be configured to show the status of OTSD and OTW or either one or non of them.}
	\label{fig:statreg}
\end{figure}
