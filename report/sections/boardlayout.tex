...
While the power board is referenced throughout this section, the discussion of the power board can be found in section \ref{sec:powerboard}.
...
In section \ref{sec:inverter} the maximum current to be expected in each phase is found to be 300$A$.
...
\begin{figure}[!h]
	\centering
	\includegraphics[width=.75\linewidth,trim=0cm 1.5cm 0cm 2cm]{graphics/torque_pedal_downscale}
...
By reading the drive enable signal it is therefore possible to determine when the controller should be running.
...
This results in approximately 1\si{\ampere} current.
