...
\end{equation}
\begin{equation}
\rightarrow K_p=(3 \alpha - \frac{K_v}{J}- \frac{R}{2L})L
\end{equation}\newline
\begin{equation}
...
\begin{equation}
\frac{I_q}{I_q-ref}=\frac{\frac{K_t}{L_q}}{s^2+s(\frac{R_q}{L_q}+\frac{K_p}{L_q})+\frac{K_t}{L_q}}
\end{equation}
...
Solving for the alpha values we find:
\begin{equation}
...
\begin{equation}
y(n) = y(n-1) + G\cdot (t(n)-t(n-1)\frac{n+u(n-1)}{2}
\end{equation}

Here n is the time step, G is the gain value, t is the time and u is the function to integrate. The Trapezoidal method yields a more precise method than forward or backwards Euler methods, due to it being 2nd order precise ($O(h^2)$), but it takes twice the computation power.
