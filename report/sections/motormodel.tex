...
Controlling the PMSM\todo[inline]{Erlingur - what ever we want to call the motor. } motor will be done using the principles of vector control\todo{Martin: Field Oriented Control. Should Clarke Park be explained, or just referenced?}, where the controller produces a change to the PWM inverter input. This is done by monitoring the two of the three current inputs to the motor and transforming them using the dq0 transform. This transform yields a d and q DC value and a zero, which are respectively referred to a torque demand and a flux demand. \newline
...
To properly control this, a mathematical model of the PMSM motor from section 7 is required to derive controller values for the two PID controllers\todo[inline]{erlingur - which two PID cpntrollers? in the figure?}. To model the PMSM motor an equivalent circuit of the motor will be used. For an AC electrical motor the equivalent circuit can be represented by the diagram presented on figure \ref{fig:PMSM_Equivalent_d_axis} and \ref{fig:PMSM_Equivalent_q_axis}.\newline
>>>>>>> fc976f2978e24ae58fd6434c622b02ccd02d0a17
...
A state space model is derived from the physical representation of a PMSM motor, figure \ref{fig:State_space_model1}, consisting of an electrical part on the left and a mechanical part on the right side of the model. The model is made in MatLab Simulink\todo[inline]{Erlingur - Not sure, but when referring to Matlab Simulink you might need a citation }. The model is very similar to that of a DC motor when interpreted like this. A wind resistance has been added to show how it affects the torque of the motor. The value of the wind resistance is the rotational speed squared multiplied by a drag value\todo[inline]{Erlingur - might need to show the drag force eq}. This value will be a negative number, which is then added to the torque output to slow down the motor. The drag can be approximated by looking at the gearings of the kart and the motors own drag coefficient.

\begin{figure}[H]
	\centering
	\includegraphics[width=.95\linewidth,trim=4cm 7cm 4cm 0cm]{graphics/full_order_with_resistances_to_pdf}
...
=======
Reducing this model\todo[inline]{Erlingur - what model? should refer to the figure here} to use transfer functions for the electrical and mechanical parts results in a more simple model with fewer loops. This model removes torque load and wind resistance. It is shown on figure \ref{fig:State_space_model3}. It can be used to find controller values if the torque load and wind resistance are seen as outside disturbances. The controller should be able to handle the amount of disturbance that can be found in these values. The wind resistance will get proportionally larger the faster the kart is moving, but for this first model it will not be taken into account. In the electric part of the circuit a new value $TS$ has been added to the denominator of the transfer function, this value is the electromagnetic time constant of the stator, defined as $TS=L_a/R_a$. The constant defined $J_{pro}$ is the projected inertia of the kart, based on the expected weight with a driver and also adding the inertia of each gearing cog.
>>>>>>> fc976f2978e24ae58fd6434c622b02ccd02d0a17
...
This \todo[inline]{Erlingur - In general, when you start a "conversation" about a figure, you should refer to it. this is confusing thus the reader needs to go back to find out what "this" is referring to}shows that the current ramps up and stays there. It has a slope at time zero+, unlike the other reduced order model. This model could be used with a controller, but will require some simulations with the motor after a controller has been developed, the downward slope of the full order model is caused by the mechanical system getting saturated by current, this should not change how each model is affected by current control, because a fast and robust controller is desired.


\subsection{Controlling Current}
The motor must be current controlled in order to limit the torque. If the motor was speed controlled, as is often used in motor control, it could demand a much higher current than possible by the circuit, requiring a limiter on the current input instead, or otherwise\todo[inline]{Erlingur - maybe instead of "otherwise" it shæuld be "or in worst case"?} it could result in destroying the drive circuit. A current control also means that the speeder pedal will be a current reference, translating into a torque reference, resulting in an even motor speed even\todo[inline]{erlingur - should this be a constant speed, there are to any even?} if the kart is driving on a uneven plane such as a hill.

A control loop is added to the model, figure \ref{fig:State_space_model4}. It must be current controlled to control the torque. This means that a feedback loop is added from inside the motor model and fed back to the controller. On this figure it is shown with the full order system.\todo[inline]{Erlingur - The reference should be here, same as before. Maybe it is just me but I don't like "on this figure" that is something that belongs in the captions}
...
To find the controller values the Stephen Dodds Settling Time Formula for $5\%$ settling time is used\todo[inline]{Erlingur - citation, and refer to the eq, it has not been introduced before, I think.}. The desired characteristics polynomial of a transfer function for the settling time formula to work is
...
The transfer function including these values result in the pzmap of figure \ref{fig:PZmapIPcontroller1}. Since there is no zero\todo[inline]{Erlingur - "Since there are no zeros" maybe?} in the transfer function, there wont be one when adding an IP controller. The added pole becomes a complex conjugate pair as there is nothing to keep them on the real axis. This means that the system is stable but might have oscillation. This will be tested in a simulation in the next section.
...
Figures \ref{fig:Output_Full} and \ref{fig:Output_LC}\todo[inline]{Erlingur - it is enough to refer to the whole figure no need to to a) and b) at this point } shows how both models perform with the controller values found above and the input signal. The full order model has a small steady state error of slightly less than $5A$ while the reduced order model is spot on. The response time for both is higher than the $0.05$ value set for the settling time formula though. The full order model settles in $0.9s$ while the reduced model settles is $0.13s$. They both handle the control as they should, with no overshoot at the start, or undershoot at the end of the breaking.
...
Discretizing the controller is required to use it in the embedded software\todo[inline]{Erlingur - maybe explain why, just a "do to something"}. To do this, the values of $K_p$ and $K_i$ can be used as they are, but for both a PI and IP controller the integrator must be discretized. An integrator in discrete is done by using a numerical integration method. The choice here will be the Trapezoidal method, which has the output equation given as: 
