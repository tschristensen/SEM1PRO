...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{.45\linewidth}
		...
		\caption{Simulation of full order model}
		\label{fig:Full_Vs_Reduced1_Current}
	\end{subfigure}
	\hspace{0.5cm}
	\begin{subfigure}[t]{.45\linewidth}
		\includegraphics[width=\textwidth]{graphics/Full_Vs_Reduced1_Speed}
		\caption{Simulation of reduced order model}
		\label{fig:Full_Vs_Reduced1_Speed}
	\end{subfigure}
	\caption{Matlab Simulink of each model with the same input}
\end{figure}

 A problem shows up by these simulations. The speed looks fine for the reduced order model but the current is not defined at zero. This is due to the removal of the inductor and integrator which removes an instance of zero being multiplied by $\tfrac{1}{R}$. The model not being defined at zero means that it is not a good idea to use it to develop a controller. The amplitude of speed and current are not important at this time, as that will be controlled later. 
...
This shows that the current ramps up and stays there. It is defined in zero, unlike the other reduced order model. This model could be used with a controller, but will require some simulations with the motor after a controller has been developed.
...
Finding the controller values for this model is done as it was for the full order model. However an IP controller will be used instead to remove a zero from the transfer function compared to the PI controller. The IP controller places the $K_p$ value in a feedback loop resulting in the following diagram
...
\begin{equation}
\frac{I_q}{I_{q-ref}}=\frac{\frac{K_t}{L_q}}{s^2+s(\frac{R_q}{L_q}+\frac{K_p}{L_q})+\frac{K_t}{L_q}}
\end{equation}
...
The characteristics equation has the form:
\begin{equation}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.65\linewidth]{graphics/Control_signal_input}
	\caption{Input reference for the controller}
	\label{fig:Input_ref}
\end{figure}

Figures \ref{fig:Output_Full} and \ref{fig:Output_LC} shows how both models perform with the controller values found above and the input signal. The full order model has a small steady state error of slightly less than $5A$ while the reduced order model is spot on. The response time for both is less than the $0.05$ value set for the settling time formula. The full order model settles in $0.9s$ while the reduced model settles is $0.13s$. They both handle the pedal release as they should, with not undershoot at the end of the breaking.
