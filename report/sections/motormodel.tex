...
\todo[inline]{Erlingur - what ever we want to call the motor. } motor will be done using the principles of vector control
\todo{Martin: Field Oriented Control. Should Clarke Park be explained, or just referenced?}
, where the controller produces a change to the PWM inverter input. This is done by monitoring the two of the three current inputs to the motor and transforming them using the dq0 transform. This transform yields a d and q DC value and a zero, which are respectively referred to a torque demand and a flux demand. \newline
...
To properly control this, a mathematical model of the PMSM motor from section 7 is required to derive controller values for the two PID controllers
\todo[inline]{erlingur - which two PID cpntrollers? in the figure?}.
 To model the PMSM motor an equivalent circuit of the motor will be used. For an AC electrical motor the equivalent circuit can be represented by the diagram presented on figure \ref{fig:PMSM_Equivalent_d_axis} and \ref{fig:PMSM_Equivalent_q_axis}.\newline
...
The components shown are: the mutual inductance $l_s$, $L_md$ and $L_mq$. Armature resistance $R_s$ and flux leakage for both d and q axes.
...
\todo[inline]{Morten: Some of this text might need better explaining.}

\subsection{State Space Model}
A state space model is derived from the physical representation of a PMSM motor, figure \ref{fig:State_space_model1}, consisting of an electrical part on the left and a mechanical part on the right side of the model. The model is made in MatLab Simulink
\todo[inline]{Erlingur - Not sure, but when referring to Matlab Simulink you might need a citation }.
 The model is very similar to that of a DC motor when interpreted like this. A wind resistance has been added to show how it affects the torque of the motor. The value of the wind resistance is the rotational speed squared multiplied by a drag value
 \todo[inline]{Erlingur - might need to show the drag force eq}.
  This value will be a negative number, which is then added to the torque output to slow down the motor. The drag can be approximated by looking at the gearings of the kart and the motors own drag coefficient.

\begin{figure}[H]
	\centering
	%\includegraphics[width=.95\linewidth,trim=4cm 7cm 4cm 7cm]{graphics/full_order_with_resistances}
...
>>>>>>> 70f2e63add82d4110c7abe3eacdf9e38fd61088c

\begin{figure}[H]
	\centering
	\includegraphics[width=.95\linewidth,trim=4cm 7cm 4cm 0cm]{graphics/full_order_as_tf_without_resistances_to_pdf}
...
\begin{figure}[H]
	\centering
	%\includegraphics[width=.95\linewidth,trim=4cm 7cm 4cm 0cm]{graphics/electrical_reduced_order_to_pdf}
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{.45\linewidth}
		\includegraphics[width=\textwidth,trim=4cm 9cm 4cm 9cm]{graphics/full_order_model_pz_map}
		\caption{PZ map of full order model}
		\label{fig:PZmap_full}
	\end{subfigure}
	\hspace{0.5cm}
	\begin{subfigure}[t]{.45\linewidth}
		\includegraphics[width=\textwidth,trim=4cm 9cm 4cm 9cm]{graphics/reduced_order_model_pz_map}
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{.45\linewidth}
		...
		\caption{Simulation of current for full and reduced order models. Time[s]/Current[A]}
		\label{fig:Full_Vs_Reduced1_Current}
	\end{subfigure}
	\hspace{0.5cm}
	\begin{subfigure}[t]{.45\linewidth}
		\includegraphics[width=\textwidth]{graphics/Full_Vs_Reduced1_Speed}
		\caption{Simulation of speed for full and reduced order models (almost stacked on each other). Time[s]/Speed[rad/s].}
		\label{fig:Full_Vs_Reduced1_Speed}
	\end{subfigure}
	\caption{Matlab Simulink of each model with the same input}
\end{figure}
\todo[inline]{Erligur - the x and y axises are missing labels. }
...
\begin{figure}[H]
	\centering
	\includegraphics[width=1\linewidth,trim=5cm 6cm 5cm 6cm]{graphics/mechanical_reduced_order_to_pdf}
	\caption{Reduced order PMSM motor model version 2.}
	\label{fig:State_space_model_reduced_order_v2}
\end{figure}

This model is basically an LR circuit where the output is the current between L and R. The current will just go as high as the L and R allows and stay there, since there is no disturbance or mechanical parts included. A PZ map should shows that the slow pole to the right is removed. Figure \ref{fig:PZmap_reduced_v2} shows the result.

\begin{figure}[H]
	\centering
	\includegraphics[width=.55\linewidth,trim=5cm 4cm 5cm 4cm]{graphics/reduced_order_model_v2_pz_map}
...
\todo[inline]{Erlingur - same thing with missing labels on axises}
This 
\todo[inline]{Erlingur - In general, when you start a "conversation" about a figure, you should refer to it. this is confusing thus the reader needs to go back to find out what "this" is referring to}
shows that the current ramps up and stays there. It has a slope at time zero+, unlike the other reduced order model. This model could be used with a controller, but will require some simulations with the motor after a controller has been developed, the downward slope of the full order model is caused by the mechanical system getting saturated by current, this should not change how each model is affected by current control, because a fast and robust controller is desired.


\subsection{Controlling Current}
The motor must be current controlled in order to limit the torque. If the motor was speed controlled, as is often used in motor control, it could demand a much higher current than possible by the circuit, requiring a limiter on the current input instead, or otherwise
\todo[inline]{Erlingur - maybe instead of "otherwise" it shæuld be "or in worst case"?}
 it could result in destroying the drive circuit. A current control also means that the speeder pedal will be a current reference, translating into a torque reference, resulting in an even motor speed even
 \todo[inline]{erlingur - should this be a constant speed, there are to any even?}
 if the kart is driving on a uneven plane such as a hill.
...
\todo[inline]{Erlingur - The reference should be here, same as before. Maybe it is just me but I don't like "on this figure" that is something that belongs in the captions}

\begin{figure}[H]
	\centering
	\includegraphics[width=.95\linewidth,trim=3cm 4cm 3cm 4cm]{graphics/full_order_as_tf_with_controller_to_pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.95\linewidth,trim=3cm 4cm 3cm 4cm]{graphics/full_order_rearranged_to_pdf}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.95\linewidth,trim=4cm 3cm 4cm 3cm]{graphics/plant_with_pi_to_pdf}
...
\begin{equation}
\frac{I_q}{I_{q-ref}}=G(s)=\frac{2 J_m s + 2 K_v}{s^2(2 J_m L_a) + s(2 K_v L_a + 2 J_m R_a)+3 K_t^2 + 2K_v R_a}
\label{eq:full_order_tf}
\end{equation}

Analysing this transfer function it can be seen that we have a second order system, with possibly one zero and one or two poles. The zero will be located at the root of the nominator: $2J_ms+2K_v=0 \rightarrow s=\frac{-K_v}{J_m}$. Which will probably be a very small real number, as inertia should be much larger than the viscous friction. 
The pole(s) will be located at the roots of the denominator. The denominator can be reduced by dividing everything with $2J_{pro}L_a$. yielding the transfer function:

\begin{equation}
\frac{I_q}{I_{q-ref}}=G(s)=\frac{\frac{1}{L_a}s+\frac{K_v}{J_pro}}{
...
\end{equation}
\todo{Martin: I'm quite happy, that I didn't have to put this fucking equation in}
...
To find the controller values the Stephen Dodds Settling Time Formula for $5\%$ settling time is used
\todo[inline]{Erlingur - citation, and refer to the eq, it has not been introduced before, I think.}.
 The desired characteristics polynomial of a transfer function for the settling time formula to work is
...
Based on the transfer function in equation ref and a settling time of 0.05 seconds, alpha will be defined by the following equation. This value is used to find the Kp and Ki values for the controller.
...
Solving the above equation for $d_2$ and $d_1$ 
...
\begin{equation}
3 \alpha= \frac{R}{2L}+ \frac{K_v}{J}+ \frac{K_p}{L}
\end{equation}
\begin{equation}
\rightarrow K_p=(3 \alpha - \frac{K_v}{J_m}- \frac{R_a}{2L})L
\end{equation}\newline
\begin{equation}
3 \alpha^2= \frac{2K_t^2}{2JL}+ \frac{K_vR}{JL}+ \frac{K_i}{L}+ \frac{K_pK_v}{2J}
\end{equation}
\begin{equation}
\rightarrow K_i=(3 \alpha^2- \frac{K_pK_v}{2J}- \frac{K_vR}{JL}- \frac{3K_t^2}{2JL})L
\end{equation}
...
These equations yield the following controller values when inserting the known motor parameters. This is with a settling time of 0.05 seconds. Equation \ref{eq:Kpvalue} and \ref{eq:Kivalue}.
\begin{equation}
K_p=0.0111
\label{eq:Kpvalue}
\end{equation}
...
\label{eq:Kivalue}
\end{equation}
\todo[inline]{Erlingur - maybe I am not noticing, but I think that a lot of the above eq's are not referred to in the text }
The pole-zero map for the transfer function including the controller confirms that a new pole and zero has been added. All poles and zeroes lie on the real axis, meaning they affect the system by altering the decay. The system is stable with these values since they are all on the left side of the imaginary axis. See figure \ref{fig:PZmap_Control_full}.

\begin{figure}[H]
	\centering
	\includegraphics[width=.55\linewidth,trim=4cm 3cm 4cm 3cm]{graphics/full_order_w_pid_to_pdf}
	\caption{PZ map with the controller}
	\label{fig:PZmap_Control_full}
\end{figure}

\todo[inline]{Morten: Very hard to see the pole and zeroes- don't know how to improve it. matlab hates me today:sadface}
...
The same procedure as for the full order system will be used for this transfer function. The overall transfer function using the diagram of figure \ref{fig:State_space_model_reduced_order_v2} and Masons Rule becomes:

\begin{equation}
\frac{I_q}{V_{in}}=H(s)=\frac{1}{R_a+L_qs}
\end{equation}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.95\linewidth,trim=4cm 3cm 4cm 3cm]{graphics/plant_with_ip_to_pdf}
	\caption{Reduced order system with an IP controller}
	\label{fig:IPcontroller1}
\end{figure}

The transfer function of this, including the model then becomes:
\begin{equation}
\frac{I_q}{I_{qref}}=\frac{\frac{K_t}{L_a}}{s^2+s(\frac{R_a}{L_a}+\frac{K_p}{L_a})+\frac{K_t}{L_a}}
\end{equation}

Using Dodd's $5\%$ settling time formula and solving using a rank of 2nd order and a settling time of $0.05s$. The results are:
\begin{equation}
\alpha=\frac{1.5(1+2)}{0.05}=90
\end{equation}

The characteristics equation has the form of the following equation, showing how the alphas are inserted into the characteristic equation.
\begin{equation}
s^2+sd_1+d_0 = s^2+2 \alpha s+ \alpha^2
\label{eq:char_eq_reduced1}
\end{equation}

Solving for the alpha values $K_p$ and $K_p$ can be found:
\begin{equation}
...
\end{equation}
\todo[inline]{Erlingur - same thing with the eq's a lot of numbered eq's that are not referred to inthe text}
The transfer function including these values result in the pzmap of figure \ref{fig:PZmapIPcontroller1}. Since there is no zero
\todo[inline]{Erlingur - "Since there are no zeros" maybe?}
in the transfer function, there wont be one when adding an IP controller. The added pole becomes a complex conjugate pair as there is nothing to keep them on the real axis. This means that the system is stable but might have oscillation. This will be tested in a simulation in the next section.

\begin{figure}[H]
	\centering
	\includegraphics[width=.55\linewidth,trim=4cm 3cm 4cm 3cm]{graphics/reduced_order_model_w_ip_controller_to_pdf}
...
\todo[inline]{Erlingur - MAybe there should be a table with the gains?}
...
Figures \ref{fig:Output_Full} and \ref{fig:Output_LC}
\todo[inline]{Erlingur - it is enough to refer to the whole figure no need to to a) and b) at this point }
shows how both models perform with the controller values found above and the input signal. The full order model has a small steady state error of slightly less than $5A$ while the reduced order model is spot on. The response time for both is higher than the $0.05$ value set for the settling time formula though. The full order model settles in $0.9s$ while the reduced model settles is $0.13s$. They both handle the control as they should, with no overshoot at the start, or undershoot at the end of the breaking.
...
The controller to be used for the kart will be determined by running a more in depth simulation in the next section, this simulation will simulate the physical systems, breaks, torque etc. A choice will be made based on controller performance in these tests.


\subsection{Discretization}
Discretizing the controller is required to use it in the embedded software
\todo[inline]{Erlingur - maybe explain why, just a "do to something"}.
To do this, the values of $K_p$ and $K_i$ can be used as they are, but for both a PI and IP controller the integrator must be discretized. An integrator in discrete is done by using a numerical integration method. The choice here will be the Trapezoidal method, which has the output equation given as: 
