...
A plate is atteched behind the seat to fit all of the power electronics and drive systems that are designed throughout this project.
To the right of the driver a mount is positioned for the motor.
The mount is protected by a metal cover. 
In order to transfer rotation to the rear wheels, a gearing is placed between the motor and the rear axle.
A conventional disc brake comes pre-installed on the kart with the break pedal connected using a hydraulic connection.
The speeder pedal is also installed, but with no mechanical effect on the system.
...
Four of these batteries will be put in series, yielding a combined nominal voltage of 52.8V and a discharge current of 560A. 
The discharge current can go as high as 1200\si{\ampere} for one second.
The batteries are relatively small at $\approx 238x120x82\si{\milli\meter}$ and $3.2\si{\kg}$ of weight each.
...
\todo[inline]{Thomas: I would argue that the dimensions of the batteries are irrelevant}

\subsection{PMAC Motor}
The PMAC motor provided is a three phase motor capable of producing 6 hp continuously and a peak power of 19 hp for a minute, at a maximum rotational speed of 5000 rpm. 
It has a high efficiency of 90 percent and a stall torque of 38 Nm. Its typical uses are, among other things, marine, robotics and go-karts.
At 19 hp and 5000 rpm, it is comparable with engines typically used for go karts recreational use, but its speed is significantly lower.
The power curve of the motor is also likely different from a gasoline engine, but there's no way to compare them.
\todo[inline]{Is power curve relevant?  - Mikkel}
\todo[inline]{Thomas: Is recreational use legal?}
...
By nature a variable resistor will be noisy therefore filtering the signal will be necessary.
...
\todo[inline]{Thomas: What other projects use this go-kart?}
