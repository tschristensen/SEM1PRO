...
Both Simulink and Plecs Standalone have been used, as they excel at different purposes. 
Simulink is used to test the controller on a system with as much realism as possible to make sure the go-kart functions, whereas Plecs is used to simulate the inverter to determine whether there will be heating issues during operation.
...
In the Simulink model, the tyre is modelled by a friction block in series with the rotor, and the brake is modelled in a subsystem, as seen on figure~\ref{fig:simulations_mechanical_simscape}.

\begin{figure}[H]
	\begin{center}
		\includegraphics[width = 12cm]{graphics/simulations_mechanical_simscape.png}
		\caption{Block diagram for the mechanical system used in Simulink.}
...
Both Simulink and Plecs have built in models of PMAC motors.
...
output = \left\lfloor \Bigg( \frac{\theta \cdot 256}{2 \pi} + 0.5 \Bigg) \% 256\right\rfloor
\end{equation}
...
The PI controllers produce duty cycle values in the range of 
$\pm \tfrac{2}{\sqrt{(3)}}$, although it is possible to exceed this range at this point in the diagram.\\
...
The majority of this block diagram calculates the inverse Clarke Park transformation, and in the upper right corner, the third harmonic injection is calculated using the knowledge given in~\ref{sub:third_harmonic_injection}. \\
...
This is displayed on figure~\ref{fig:torque_input}.
\begin{figure}[!h]
	\begin{subfigure}[b]{.49\linewidth}
	\centering
	\includegraphics[width = \linewidth]{graphics/torque_input}
	\caption{Torque step input for the tests performed in this section}
	\label{fig:torque_input}
	\end{subfigure}
	\begin{subfigure}[b]{.49\linewidth}
	\centering
	\includegraphics[width=\linewidth]{graphics/Sim_results_pi_Idq}
	\caption{d and q current simulated with a positive and negative step.}
	\label{fig:Sim_results_pi_Idq}
	\end{subfigure}
	\caption{Torque input and simulated current output.}
\end{figure}
...
For controller frequency, the error happens when the motor is running at 2250rpm.
...
\begin{figure}[!h]
	...
	\caption{Voltage waveforms measured when the motor is running at 4000rpm.}
...
\begin{figure}[H]
	...
	\caption{Input to $K_p$ gain blocks are connected to the $I_{dq}$ feedback wire instead of the output of the summation block.}
\end{figure}

The parameters are divided by half the battery voltage, and shown in equation~\ref{eq:IP_controller_constants}.

\begin{equation}
K_p = -2.65\cdot 10^{-5} \quad %1515151515156e-05 
K_i = 1.23\cdot 10^{-2} %0.012272727272727
\label{eq:IP_controller_constants}
\end{equation}

Figure~\ref{fig:Sim_results_ip_Idq} shows the d and q-axis current as the controller is given a positive and a negative step, as seen on figure~\ref{fig:torque_input}.
...
The steady state error of the q-controller is 10\%, while the d-controller has a steady state error of $16\si{\ampere}$.
...
Since neither of the two controller designs work optimally, the PI controller will be redesigned with a settling time of $10\si{\milli \second}$.
The controller is designed for a load of $200\si{\kilogram}$.
...
The rising and falling rates are set to $10 \si{\kilo \ampere \per \second}$ and $-10 \si{\kilo \ampere \per \second}$.
...
The faster $I_{q}$-controller is much more precise.
The rate limiter ensures, that the overshoot is less than 1 percent.
The steady state error negligible.
However, the $I_{d}$-controller is more noisy, but still close to zero.
...
The IP controller adds a pole to the systems, which makes the fast pole on figure~\ref{fig:PZmap_full} complex conjugate poles that can oscillate.
...
At 20\si{\kilo\hertz}, this means, the whole circuit needs to be solved 120.000 times per second, so using plecs is still not fast.
...
The blue rectangle around each mosfet is a heat sink, that absorbs all the power loss within it.
...
It should be noted, that the heat sink box includes all losses inside, meaning it includes both the loss specified in the thermal model, but also the loss due to a non-zero on-resistance. 
This is part of the reason, the on-resistance has been set to zero, the other part being faster simulation. \\

All heat sink boxes are connected through a thermal resistor (the blue ones) to a shunt thermal capacitor (CthG).
...
CthG along with Heatsink Thermal Resistance models the 30x30 heat sink used to mount the inverter. 
...
\begin{figure}[H]
	\begin{center}
		\includegraphics[angle = 90,width = 0.63\linewidth]{graphics/Plecs_electrical}
...
In series with this is a non-ideal wire with an inductance and resistance.
...
Figure~\ref{fig:plecs_power_loss} shows the heat transfer to the heat sink.
...
Not only is this power loss damaging for the efficiency of the inverter, it also produces a lot of heat, that is not absorbed by the heat sink.
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{.49\linewidth}
		...
		\caption{Temperature increase of the heat sink.}
...
Figure~\ref{fig:plecs_heatsink_temperature} displays the heat sink temperature  during the full throttle test
