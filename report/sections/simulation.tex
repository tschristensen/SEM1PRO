...
\end{itemize}
\todo[inline]{Thomas: These are events and should be written as such: "Actuation of the torque pedal, Saruration of supply voltage" and so on. Short and precise.}
...
\todo[inline]{Thomas: Our interest in tools we do not have access to is irrellevant, suffice it to say that the mechanical library is used. Also.. Another two "we"'s?!?!}
\begin{figure}[H]
...
\todo{Thomas: So, a model of the kart?}
...
It is possible to add inertias that are connected to the same mechanical rod. 
\todo{Thomas: Okay, that's cool but.. Do we care?}
...
Radius is defined by the number of teeth of the gear, G1, and the pitch, which is the distance between two adjacent teeth, which is 12.5 mm.
Hence the radius can be calculated by equation \todo{Thomas: What equation?}.
Inertia of a disc depends on mass and radius of the disc.
\todo[inline]{Thomas: Is G=G1? Also, assign a variable to pitch, text in an equation is not pretty.}
\begin{equation}
...
\end{equation}
\todo[inline]{Thomas: I believe the last sentence in the above paragraph references this equation but it is unclear. Also, is G\^4 = pow(G,4)?}
...
This ratio, G2/G1 is put into the block "Wheel and Axle".

The mass is set to 200 - 250 kg, depending on the driver, and this includes the mass of the car.
This is a high estimate. 
A lower value could make the controller less stable, but a higher value will heat up the inverter more.
\todo[inline]{Thomas: This is probably true but i would like more of a discussion about why you chose a high estimate, knowing full well that this would put higher thermal stress on the inverter?}

\todo[inline]{Thomas: A short introduction to the following would be reasonable: "Also the friction caused by the wind resistance is simulated. This can be done...."}
...
The frontal area has been approximated to two boxes with the combined area of 0.6\si{\metre\squared}\todo{add a picture of a go kart with two boxes across it, the lower one being 1 m wide, and 0.4 m high, and the second one being 0.5 m wide an 0.4 m high. mba.}. 
The density of air is approximately 1.225 \si{\kilo\gram\per\metre\cubed}, $c$ is approximately 0.8, according to a paper about air resistance found online. \todo[inline]{Thomas: Ofcourse you have a reference to this paper? If not, find one and add it to the bib.tex file.}
...
These constants are put into the gain block, "Drag coefficient", and multiplied by the square of the speed. \todo{Thomas: It has been a while since you discussed the block diagram, i think you should mention the figure again, just for good measure.}
The result is put into an ideal force source. 
...
This system can be simplified, so that all inertia is combined in one block and the gear and wheel can be removed. 
This is done by a set of rules that apply for this mechanical circuit: The gear box reduces the speed, and increases torque. 
Torque is force times radius, and speed is angular velocity times radius.
...
For a mass of 250 kg, this comes to 0.282\si{\kilo\gram\metre\squared}.

In equation~\ref{eq:wind_resistance}, speed can be replaced with angular velocity and a gain, and force can be replaced by torque and a gain. So the equation becomes this:

\begin{equation}
\frac{T G2}{r G1} = c_{drag} \big(\omega r \tfrac{G1}{G2}\big)^2
\end{equation}

Isolating $T$, and putting $\omega$ by itself, we get:
\todo{Thomas: Poor omega :'(}
\begin{equation}
...
\todo{Thomas: These are technical details that are completely irrelevant in relation to the simulations.}
...
The tire block consists of static, dynamic and viscous friction. 
This is a simplification of how tires actually work, \todo{Thomas: Enlighten me? How do they actually work?} but for this project it is sufficiently accurate. 
...
If the speed is reduced to below some threshold, the torque will be reduced linearly towards 0. 
...
\todo[inline]{Thomas: And this is not a real world thing?}
...
The parameters can be seen on figure~\ref{fig:pmsm_parameters_simscape}.
\todo[inline]{Thomas: I would prefer a table.} 
...
The advantage in Plecs is its ability to simulate switch mode power electronics, where there is a vast ratio between the minimum timestep defined by the switching frequencies, and the duration of the simulation. \todo[inline]{I am not sure i understand why this ratio is important?}
...
Current is sensed on wires A and B, and used to calculate $I_C$ in the upper part of the block diagram. 
The signal goes to a Clarke-Park transformation block outside figure~\ref{fig:simulations_electrical}, and returns as "\texttildelow" $I_d$ and $I_q$ on the bottom left part of the picture. \\

The angular position is measured with an ideal position sensor, and then sent to the encoder block.
Here, the finite precision of the encoder is simulated by "\texttildelow" equation~\ref{eq:Encoder_block_function}

\begin{equation}
\label{eq:Encoder_block_function}
output = \left\lfloor\frac{\phi \cdot 256}{2 \pi} \% 256\right\rfloor
\end{equation}
...
The floor function rounds a number, effectively quantizing the output. 
