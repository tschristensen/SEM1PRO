...
The purpose of that is to wrap \todo{Wrap is the common way of saying, if a number goes outside a range, it restarts from the other end of the range, like an integer overflow is (ofteh) handled in coding.} the output to a value between 0 and 255, which is the same range of integers as the real encoder. 
...
To reduce computation time for the Zybo or enable it to be done in FPGA, all six trigonometric functions in equation~\ref{eq:sim_clark-park} can be saved in six lookup tables without any loss of precision. 
However, Simulink seems to employ an inefficient method of looking up these values, which means simulation time actually is three times longer.\\
...
This is done in the PWM generation subsystem, which is shown on figure~\ref{fig:sim_PWM_generation}.
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 0.7\linewidth]{graphics/sim_results_pi_controller}
	\caption{Output of the PI controllers}
	\label{fig:sim_results_pi_controller}
\end{figure}
...
\todo[inline]{Thomas: This needs something more.. Martin: ya think?}
\begin{figure}[H]
	\begin{center}
