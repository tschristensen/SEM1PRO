...
The mechanical system consists of a mass of the gokart with driver, turbulent air resistance, wheels and a gear. While there are a lot of interesting blocks in the Simscape $\rightarrow$ SimDriveline library, we will stick with the mechanical library, because this is detailed enough, and more easy to port to Plecs. The simulations do not take into account for wheel spin.
...
where $\rho$ is the density of air, A is the frontal area, c is the drag coefficient and v is the speed. The frontal area has been approximated to two boxes with the combined area of $0.6 m^2$\todo{add a picture of a go kart with two boxes across it, the lower one being 1 m wide, and 0.4 m high, and the second one being 0.5 m wide an 0.4 m high.}. The density of air is approximately $1.225 \frac{kg}{m^3}$, and the constant c is approximately 0.8, according to a paper about air resistance found online. The constants are multiplied into one constant called c\_drag, as shown in equation~\ref{eq:cdrag}
...
\begin{figure}[h]
	...
	\caption{Block diagram of the Simulink electrical network and modulation. Red indicates fixed time steps.}
...
The angular position is measured with an ideal position sensor, and then multiplied by the number of pole pairs to get the electrical position. in order to model the low resolution of the encoder, a quantizer block was placed after the "ps--s" block, but this causes stiffness to the point where the simulations almost stall. Instead this has been achieved with look-up-tables inside the controller described in section~\ref{sub:sim_controller}

After the controller block, there are four blocks manipulating the dq signals. The controller outputs a vector\todo{vector in this case is matlabs way of saying array. footnote?} of length 3. Values range from -1.15 to 1.15, corresponding with the "duty cycle". This is transformed from dq0 to abc, then overmodulated with third harmonic injection, and multiplied with half the battery voltage. This means, a value of 1 from the overmodulation block corresponds to 26.4 and a value of -1 is -26.4, which in turn means, that the only current going through the "very large resistor" is cased by the third harmonic. Finally the output is saturated to -26.4 and 26.4 to ensure the voltage does not exceed what the battery can generate. 

The setpoint value for the controller's q current is defined in the signal builder block. The signal builder is a in torque, which is converted to current by dividing with $K_T \tfrac{3}{2}$. 