...
The constants, Kp and Ki, are calculated in chapter~\ref{sec:controller_design}, and divided by half the battery voltage.
...
\todo[inline]{Martin: I'm actually not sure about the implemented controller.}
...
It does noting to reduce slip, but the transition from slipping and non-slipping is handled rather well.\\
...
\begin{figure}[H]
	...
	\caption{Input to Kp gain blocks are connected to the Idq feedback wire instead of the output of the subtraction 'circle'}
...
The steady state error of the q-controller is 10 \%, while the has a stead state error of $\mathrm{16\si{\ampere}}$.
...
However, the d and q-axis current is still oscillating from the current step displayed on figure~\ref{fig:sim_results_ip_slip_acceleration_current}, so it's hard to determine if how the controller handles non-blocking braking.
...
\todo[inline]{Martin: Morten, maybe you can write something clever about why the ip controller is so bad}
...
\end{equation}
\todo{Martin: Can we quote wikipedia?}
...
12.5 \si{\nano \henry \per \centi \meter} is a good estimate, used by Morten Nymann.\\
...
\begin{figure}[H]
	...
	\caption{Current ripple in the electrolytic capacitors}
	\label{fig:plecs_current_ripple}
\end{figure}

The large current ripple would result in 450 \si{\watt} of loss, which is a lot more than anticipated.
...
the ripple is quite low, so the inverter will work optimally at most times.
...
\todo[inline]{Martin: It is now 4:12 am, and I'm too tired to continue}
