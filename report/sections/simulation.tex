...
The first inertia block contains both the inertia of the motor, which is 0.0052, \todo[inline]{potatoes? add a unit!} and the motor-side gear \todo[inline]{which is unknown for the motor side gear?}. 
...
where $\rho$ is the mass density of iron of $7870 \dfrac{kg}{m^3}$, \todo[inline]{use tfrac for in-text fractions? this one is quite big atm.} r is the radius, and h is the thickness of 7 mm.
...
This ratio, G2/G1 is put into the block "Gear Box".\todo[inline]{in what figure? reference}
...
Wind resistance has a significant effect on the speed of the kart, especially at higher speeds.
...
\end{equation}
\todo[inline]{Morten: I don't get what Jref is completely.}
...
In equation~\ref{eq:cdrag}, speed can be replaced with angular velocity and a gain, and force can be replaced by torque and a gain. So the equation becomes this: \todo[inline]{Morten: Reference !}
...
\todo[inline]{Morten: Why is plecs mentioned here. the models i see are simscape?}
...
\todo[inline]{Morten: last sentence. i don't understand where this gain is. Overall this paragraph has a lot of information, maybe a newline in between.}
...
\todo[inline]{Thomas: Irrelevant. Morten: Agreed!}
...
\todo[inline]{Thomas: Again, Irrelevant. Morten: somewhat, this intro should be rewritten slightly. doesn't matter how they are found.}
...
\end{table}
\todo[inline]{Morten: Fix table with correct lowercases for Ld Lq etc. and correct greek letters for phi etc.}


One thing to note in both  cases is that the flux linkage is divided by the number of pole pairs. \todo[inline]{Morten: i don't really get why we are looking at this. Must have better flow from the preveious section}
...
This torque is converted into the current used in the controller by multiplying with $\tfrac{3}{2 KT_T}$. \todo[inline]{Morten I write kt as $K_t$ we should uniform it.}
...
The constants, Kp and Ki, are calculated in chapter~\ref{sec:controller_design}, and divided by half the battery voltage. \todo[inline]{Morten: did this change? i think you said something}
This is done, because the controller designs produce a voltage, whereas the simulink controller produces a percentage of half the battery voltage.
Similarly the implemented controller in chapter~\ref{sec:Embedded_Design} produces a range from -1 to 1, so the controller constants need to be scaled acordingly
\todo[inline]{Martin: I'm actually not sure about the implemented controller. Morten: who knows. i don't either}
...
At first, a simulation will be run where the throttle is pressed fully from 0.12 s to 5 s, \todo[inline]{Morten: Similar to the one used in controller design section. where Kp and Ki values also are taken from} and then released.
...
There is also a small steady state error of about 2 \%. \todo[inline]{Morten: Which was expected based on the controller design where the steady state error was 5A or 1-2\%}
The noise on both the d and q current at 1.8\si{\second} is caused by the quantization of the encoder. 
It seems to cause a resonation when the motor is runing at 2250 RPM, that the Clake-park transformation can't handle.
Meanwhile, the d-axis current is kept very low, at least until 2.6 seconds, when the voltage starts saturating. 
A small error (of up to -20 A) is not a big issue, as this is likely to occur if the position encoder is not perfectly aligned.
...
When the throttle is released after 5 s, the power should ideally not become negative, as the inverter is not designed to handle regenerative braking.
...
The power peaks at $\mathrm{-400 \si{\watt}}$ for a short time.
Integrating the power from 5.06 to 5.5 s gives an energy of $\mathrm{-25 \si{\joule}}$.
...
Integrating this undershoot yields $\mathrm{-62 \si{\joule}}$. 
This is not a major concern, however it causes a ripple in the speed, that might be felt. \\

To simulate wheel spin and wheel lock, the mass of the go-kart has been lowered to 150 kg, and the weight distribution has been reduced to 50 \% on the rear wheels.
...
The brake steps to 66 \si{\newton\meter} at $\mathrm{5.36 \si{\second}}$, and ramps up until it hits a limit of $\mathrm{102 \si{\newton \meter}}$ at $\mathrm{6 \si{\second}}$. \todo[inline]{Morten: needs correct math names. also in the next bit}
...
\begin{figure}[H]
	...
	\caption{Input to Kp gain blocks are connected to the Idq feedback wire instead of the output of the subtraction 'circle'} \todo[inline]{Morten: is this sentence finished?}
	\label{fig:ip_controller_in_full_system}
\end{figure}
...
The steady state error of the q-controller is 10 \%, while the has a stead state error of $\mathrm{16\si{\ampere}}$. \todo[inline]{Moten: Sentence incomplete!}
...
However, the d and q-axis current is still oscillating from the current step displayed on figure~\ref{fig:sim_results_ip_slip_acceleration_current}, so it's hard to determine if how the controller handles non-blocking braking. \todo[inline]{Morten: some gramatical errors here.}
At 6.3\si{\second}, the brakes lock, at which time, both the d and q-axis current oscillate.
This causes the wheel deceleration to be halted at 6.5\si{\second}, as seen on figure~\ref{fig:sim_results_ip_slip_abraking}.
...
The same happens when the brake is released again.//
...
\todo[inline]{Morten: weird spacing in the equation and e-3 is not raised}
...
\begin{figure}[H]
	...
	\label{fig:Sim_results_p01i_Idq}
\end{figure}

The faster controller is much more precise.
...
There is a small current overshoot when the pedal is released of only -3\si{\ampere}. The power caused by this is shown on figure~\ref{fig:Sim_results_pi01_power}
...
\todo[inline]{Martin: Morten, maybe you can write something clever about why the ip controller is so bad. Morten: ok will try ! this next bit is new}

Based on the controller analysis in section \ref{sec:controller_design} where the IP controller was designed based on a reduced order model without any mechanical part, it must be concluded that the slow pole that was not included in the model, as seen on figure \ref{fig:PZmap_full} in that same section, is too important to remove by reducing the order of the model, resulting in an unstable result in this full simulation. 
The IP controller itself adds a pole to the system which moves the remaining fast pole to become a complex conjugate pair that will result in a decaying oscillating response to a step. This oscillation proves to be too much when adding the slow poles of the mechanical system. As it has been proven now, this controller is not able to account for the mechanical system as a disturbance, which is unsurprising really giving the scope of the mechanics involved.
\todo[inline]{Morten: First sentence is very long and stupid? help :P}
...
\todo[inline]{Thomas: This needs something more.. Martin: ya think? Thomas: I think... Martin: OK Fine then}
\begin{figure}[H]
	\begin{center}
...
Figure~\ref{fig:turn-on_loss_lut} shows the 2D lookup table of turn on losses as a graph.

\begin{figure}[H]
	\centering
	\includegraphics[width = 0.6\linewidth]{graphics/turn-on_loss_lut}
...
\end{equation}
\todo{Martin: Can we quote wikipedia? Morten: no. at least use the sources that are on the bottom of the wiki page.}

where $C_{Rv}$ is the volumetric heat capacity of aliminium, which is 2.422. 
...
In series with this is a non-ideal wire modelled by a resistor of 0.77 \si{\milli \ohm}, and an inductor of 6 \si{\micro \henry}.
This has been calculated by~\ref{eq:wire_calculation}.

\begin{equation}
\begin{split}
R_{wire} = \frac{1.7e-8 \si{\ohm \meter} \cdot 5 \si{\meter}}{50 \si{\milli \meter \squared}}  + 200 \si{ \micro \ohm}\\
L_{wire} = 12.5 \si{\nano \henry \per \centi \meter} \cdot 5 \si{\meter}
\label{eq:wire_calculation}
\end{split}
\end{equation}

The battery wire is a 150 square mm wire, at five meters long including the connection to all the batteries. 
The 200 \si{\micro \ohm} is the resistance of the relay.
12.5 \si{\nano \henry \per \centi \meter} is a good estimate, used by Morten Nymann.\\ \todo[inline]{Morten: whos that?- add professor of blabla f.eks.}

The E caps represent the electrolytic capacitor bank, consisting of 8 2200\si{\micro \farad} capacitors in parallel. 
Bank ESR is the parallel ESR of these 8 capacitors, of 12.25 \si{\milli \ohm}.
...
F caps represent the film capacitors. These are significantly smaller, but have no ESR, and handle higher frequencies better than the electrolytic.\\
...
\todo[inline]{Martin: Maybe this should be omitted, since we so far do not have anything about capacitor choices. Morten: maybe. leave it for now. Or simply explain why we don't have anythign about the caps}
\begin{figure}[H]
	\centering
...
The large current ripple would result in up to 190 \si{\watt} of loss, which is a lot more than anticipated.
