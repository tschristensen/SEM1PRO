...
\end{figure}
As it can be seen it was chosen to build the inverter on a large heat sink, where also the Zybo, digital and analog boards are mounted for mechanical integrity. 
Below the heat sink three fans were places to produce airflow on the heat sink for better heat dissipation.

The wires from the batteries will introduce inductance to the system. This will cause in that the current can not change instantaneously. Capacitors are added to counteract this behavior. It is chosen to add eight Panasonic Aluminum Electrolytic Capacitors are added, each having capacitance of 2200$\mu$F. Those are added to smoothen out low frequency voltage spikes. To counter for the higher frequency, another eight film capacitors with capacitance of 4.7$\mu$F are added.
