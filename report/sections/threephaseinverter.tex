...
Power inverters are circuits which transfer power from a DC source to an AC load.
They come in all shapes and sizes, from low power functions such as powering a car radio to that of backing up a building in case of power outage.
Inverters can come in many different varieties, differing in price, power, efficiency and purpose.
...
Power inverters are used today for many tasks like powering appliances in a car such as cell phones, radios and televisions.
They also come in handy for consumers who own camping vehicles, boats and at construction sites where an electric grid may not be as accessible to hook into.
Inverters allow the user to  provide AC power in areas where only batteries can be made available, allowing portability and freeing the user of long power cords.

\begin{figure}[h!]
	\centering
	\includegraphics[scale = 0.7]{graphics/inverter_blockD}
	\caption{Block diagram of inverter. Battery supply is given to the MOSFET driver where it will convert DC to AC and the resulting AC is given to the step up transformer \todo{wrong wrong wRONG WRONG WRONG!!!!!!!}}
\end{figure}

When four switches, S1 to S4, are connected to the DC power supply, S1 and S4 and also S2 and S4 are respectively paired and the pairs are alternately turned ON and OFF, the AC flows as shown in Figure~


\subsection{Principle of Inverter Operation}

An inverter is a device to create the AC from the DC power supply. Below is given a simple example in order to be understandable the operation of an Inverter. See the basic principle with the single-phase DC as the simplest example.
Figure~\ref{fig:ac_create} shows the example of the method to convert the DC to the AC by utilizing a lamp for the load in place of a motor. 

\begin{figure}[h!]
	\centering
	\includegraphics[scale = 0.7]{graphics/ac_create}
	\caption{Method to create AC}
	\label{fig:ac_create}
\end{figure}

When four switches, S1 to S4, are connected to the DC power supply, S1 and S4 and also S2 and S4 are respectively paired and the pairs are alternately turned ON and OFF, the AC flows as shown in Figure~\ref{fig:switching}

\begin{figure}[h!]
	\centering
	\includegraphics[scale = 0.7]{graphics/switching}
	\caption{ }
	\label{fig:switching}
\end{figure}

\begin{itemize}
	\item When the switches S1 and S4 are turned ON, the current flows in the lamp in the direction of A.\\
	\item When the switches S2 and S3 are turned ON, the current flows in the lamp in the direction of B
...
\subsection{The three phase inverter}
...
Using the parallel setup, the two MOSFETs will share the current equally so that each will have a maximum peak current of 150A \todo{Martin: Because if one draws more current than the other, it will raise temperature, then rds-on and then reduce current.}.
...
\begin{equation}
I_{gs} = C\dfrac{dV}{dt} \rightarrow dt = C\dfrac{dV}{I_{gs}}
\label{eq:I_gs}
\end{equation}
...
The gate to source current is known. The driver supports up to 2.3A sink and 1.7A source current.\todo{Erlingur - needs some more explanations} 
