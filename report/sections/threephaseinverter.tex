...
\todo[inline]{Martin: The end of charge voltage is the maximum. 52.8 is the average, used to indicate power capacity of the battery}
...
The MOSFET parameters in table~\ref{tab:MOSdata} seems to add up to all the requirements in table~\ref{tab:req1}. \todo{Martin: well not really. Even the silicon limit isn't enought to handle 300 A comfortably.}
...
\todo[inline]{Martin: Don't consider the Rload when looking at I\_Q}
...
\todo[inline]{Martin: I think there's a shorter way to this conclusion: Since each mosfet is on half the time, it is safe to assume that half the current out must be cunducted by each mosfet}
...
Although listed in the datasheet, the drain to source resistance $R_{DS(on)}$ only applies to the  $T_J = 25 \si{\degree \celsius}$ condition. 
...
\begin{equation}
R_{DS(on),@T_J} =  \alpha R_{DS(on),@25\si{\degree \celsius}}
\label{eq:Rdstj}
\end{equation}
...
If assumed that the junction temperature will be around $100 \si{\degree \celsius}$ one can estimate the factor being somewhere close to $\alpha = 1.6$. 
...
\begin{equation}
P_{c} = R_{DS(on),@100\si{\degree \celsius}} I_{Pi,rms}^2 =  R_{DS(on),@100\si{\degree \celsius}} \bigg(\dfrac{I_{out}}{2n}\bigg)^2
\label{eq:P_cp}
\end{equation}
\todo[inline]{Martin: Is this instantaneous power loss, or continious}
...
where $I_0$ is the drain current, $I_{Pi,rms}$, of each MOSFET from equation~\ref{eq:I_pi} and $\mathrm{V_{in}}$ is the supply voltage for the inverter. 
...
\todo[inline]{Martin: Should be an explaination of what the subscribts mean.}

The $V_{DS}$ rise and fall times $t_{rv}$ and $t_{fv}$ in figure~\ref{fig:sw_loss} are defined in the MOSFET datasheet~\cite{IRF4468PbF} as the rise and fall times $t_r$ and $t_f$ seen in figure~\ref{fig:sw_time_wavef}. The times are listed in table~\ref{tab:tf_tr}.
...
\todo[inline]{Martin: I assume, that n is still number of mosfets in parallel? This also assumes, that we can charge the gates with a constant current, which we cant with a gate resistor of 4.5 Ohm}

\subsection{Total Power loss}

The power losses are calculated for up to 10 MOSFETs in parallel using equations~\ref{eq:P_cp}, \ref{eq:P_sw} and~\ref{eq:p_loss} where the switching frequency is $f_s = 20kHz$. Figures~\ref{fig:ploss_single} and~\ref{fig:ploss_comb} show the results.
...
P_{loss} = 95*6 = 570W
\end{equation*}
\todo[inline]{Martin: We should write something about the Driver and the capacitor choices}
...
As the inverter has to work with large currents and fast switching, all parasitic inductors should be avoided.
This is generally done by avoiding long wires as they contain a considerable inductance. 
It was not possible to avoid wires completely and therefore the inverter has some wires which are kept short and are twisted together to minimize inductance. 
The inverter should have a large groundplane to avoid difference in potential across the ground. 
The developed layout can be seen in figure \ref{fig:inverter_design}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\linewidth]{graphics/inverter_placeholder}
	\caption{Placehodler....}
	\label{fig:inverter_design}
\end{figure}
As it can be seen it was chosen to build the inverter on a large heatsink, where also the Zybo, digital and analog board are mounted. 
Below the heatsink three fans were places to produce airflow on the heatsink for better heat dissipation.
\clearpage